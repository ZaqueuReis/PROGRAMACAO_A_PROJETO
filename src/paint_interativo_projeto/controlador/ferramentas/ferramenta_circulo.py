...
de desenho do círculo. Cada método representa um evento do mouse durante
a criação da figura (pressionar, mover e soltar o botão).'''

class FerramentaCirculo(Ferramenta):

    def mouse_press(self, event):
        figura = Circulo(event.x, event.y, 0, self.controlador.janela.obter_cor_borda(), self.controlador.janela.obter_cor_preenchimento(), self.controlador.janela.obter_tamanho_borda())
