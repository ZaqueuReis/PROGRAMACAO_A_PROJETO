...
de desenho do oval. Cada método representa um evento do mouse durante
a criação da figura (pressionar, mover e soltar o botão).'''

class FerramentaOval(Ferramenta):

    def mouse_press(self, event):
        figura = Oval(event.x, event.y, event.x, event.y, self.controlador.janela.obter_cor_borda(), self.controlador.janela.obter_cor_preenchimento(), self.controlador.janela.obter_tamanho_borda())
